...
public class GameViewManSubScene extends SubScene {
	...
	public String ChampDetails(Champion champ) {
		String r = "Name:" + champ.getName() + '\n' + "Current Health Points: " + champ.getCurrentHP() + '\n'
				+ "Actions Points Per Turn: " + champ.getCurrentActionPoints() + '\n' + "Mana: " + champ.getMana()
		...
		for (int i = 0; i < 5; i++) {
			for (int j = 0; j < 5; j++) {
				if (CurrentGame.getBoard()[i][j] instanceof Cover) {
					...
					ImageView CoverImage = new ImageView(((Cover) CurrentGame.getBoard()[i][j]).getURL());
	...
	public void moveChamp(Champion champ) {
		...
		root.add(DisplayChamp(champ), y, x);
	}

	public void removeChamp(int x, int y, Champion champ) {

		List<Node> inPosition = getNodesByCoordinate(x, y);
	...
	public void RemoveAllChamps() {
		...
		for (int i = 0; i < 5; i++) {
			for (int j = 0; j < 5; j++) {
				List<Node> matchingNodes = getNodesByCoordinate(i, j);
